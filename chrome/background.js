  ...
  chrome.runtime.onMessage.addListener((request, sender, sendResponse) => {
    if (request.message === "adultContentFound") {
      ...
      chrome.tabs.query({ active: true, currentWindow: true }, (tabs) => {
        if (tabs.length > 0) {
          const tab = tabs[0];
          chrome.tabs.update(tab.id, { url: `chrome-extension://eiimnmioipafcokbfikbljfdeojpcgbh/public/blocked.html?site=${request.adultContent.replaceAll(" ", "_")}&blockType=category&category=adult&extension=custom` });
        }
      });
    }
  });